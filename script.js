...
const EVOLUTIONS = {
    ...
    death_lock: [
        ...
        { id: 'evo_dl_2', name: 'Quản Chế', desc: 'Lực kéo tâm xích mạnh & xa hơn; Chiều rộng Lồng thu hẹp còn 70px (Full: 50px).', cost: 110, type: 'evo' }
    ],
    creator: [
...
class Crate {
    constructor() {
        ...
        let hasInventor = (typeof p1 !== 'undefined' && p1 && p1.weapon === 'inventor') || (typeof p2 !== 'undefined' && p2 && p2.weapon === 'inventor');
        let hasEvo1 = (typeof p1 !== 'undefined' && p1 && p1.weapon === 'inventor' && p1.hasUpgrade('evo_inv_1')) || (typeof p2 !== 'undefined' && p2 && p2.weapon === 'inventor' && p2.hasUpgrade('evo_inv_1'));
...
class ConfinementCage {
    constructor(x, y, owner) {
        ...
        this.w = isFullEvo ? 50 : (hasEvo2 ? 70 : 100);
        this.h = 50;
        ...
        crates.forEach(c => {
            if (c.active) {
                ...
                if (d < this.r) {
                    ...
                    if (c.type === 'hp') this.owner.hp = Math.min(this.owner.hp + 1, this.owner.maxHp);
...
class Player {
    ...
    triggerThunderAction(held, now) {
        if (held < 30) {
            ...
            if ((this.thunderSpears || 0) > 0) {
                ...
                // Nếu phóng hết 3 cây thương mà vẫn bấm nhấp -> Tự động kích hoạt Thu Thương tức thì mà không cần tụ lực
                this.startThunderRecall(true);
            }
    ...
    startThunderRecall(instant = false) {
        ...
        if (instant) {
            ...
            this.thunderRecallWarningTimer = 30; // 0.5s cảnh báo
    ...
    executeThunderRecall() {
        ...
        mySpears.forEach(s => {
            ...
            if (typeof explosions !== 'undefined') {
                explosions.push(new Explosion(s.x, s.y, 25, '#ffff00', 0, this));
            }
        });
    ...
    update() {
        ...
        // ==========================================
        if (this.weapon === 'necromancer') {
            // 1. Tự động hồi Linh Hồn theo chu kỳ
            // Gốc: 2 Linh Hồn / 5s (300 ticks). Evo 1: 3 Linh Hồn / 3s (180 ticks).
        ...
        // ==========================================
        if (this.weapon === 'thunder') {
            // 1. Cảnh báo Thu Thương (30 frame ~ 0.5s)
            ...
            // Visual vòng tụ lực của Thunder
            if (this.thunderHoldFrames > 0) {
                ctx.save();
                let color = this.thunderHoldFrames >= 60 ? "#ff0055" : (this.thunderHoldFrames >= 30 ? "#ffff00" : "#00ffff");
                ctx.strokeStyle = color;
                ctx.lineWidth = 2.5;
                ctx.beginPath();
                let progress = Math.min(1, this.thunderHoldFrames / 60);
                ctx.arc(this.x + this.w / 2, this.y + this.h / 2, 26, 0, progress * Math.PI * 2);
                ctx.stroke();
                ctx.restore();
            }
        }
        ...
        if (this.weapon === 'shortgunATK') {
            ...
            // 1. Dash áp sát mục tiêu
            if (this.isShortgunDashing) {
                let dy = this.shortgunTargetY - this.y;
                let dist = Math.abs(dy);
                if (dist <= 15) {
                    this.y = this.shortgunTargetY;
                    this.isShortgunDashing = false;
                    this.shortgunTelegraphTimer = 30; // 0.5s cone telegraph
                } else {
                    let step = (dy > 0 ? 1 : -1) * Math.min(dist, 22);
                    this.y += step;
                }
            }

            // 2. Cone Telegraph Countdown & Burst Fire
            if (this.shortgunTelegraphTimer > 0) {
                this.shortgunTelegraphTimer--;
                if (this.shortgunTelegraphTimer === 0) {
    ...
    triggerShortgunAttack() {
        let dirY = (this.id === 1) ? -1 : 1;
        let scanRange = 180;
        let scanX = this.x + this.w / 2;
        let scanStartY = (this.id === 1) ? this.y : this.y + this.h;
        ...
        targets.forEach(t => {
            let tx = t.x + (t.w || 30) / 2;
            let ty = t.y + (t.h || 30) / 2;
            if (Math.abs(tx - scanX) <= (t.w || 30) / 2 + 20) {
                let dy = (ty - scanStartY) * dirY;
                if (dy > 0 && dy <= scanRange && dy < nearestDist) {
                    nearestDist = dy;
                    nearestTarget = t;
                }
            }
        ...
        if (nearestTarget) {
            let targetY = nearestTarget.y + (nearestTarget.h || 30) / 2;
            let stopY = targetY - dirY * 40 - (dirY === 1 ? 0 : this.h);
            stopY = Math.max(20, Math.min(canvas.height - this.h - 20, stopY));
            this.isShortgunDashing = true;
            this.shortgunTargetY = stopY;
            if (typeof addFloatingText === 'function') addFloatingText(this.x + this.w / 2, this.y - 12, "DASH! ⏩", "#ff6600", 12);
        } else {
            this.shortgunTelegraphTimer = 10;
        }
    }
    ...
    handleShooting(inp, now) {
        ...
        if (this.weapon === 'shortgunATK') {
            ...
            if (inp.s) {
                // Double Tap detection trong lúc lao hoặc chờ cảnh báo
                if (this.isShortgunDashing || this.shortgunTelegraphTimer > 0) {
                    if (!this.shortgunDoubleTap) {
                ...
                if (this.ammo > 0 && now - this.lastShot > 400) {
                    this.ammo--;
                    this.lastShot = now;
        ...
        if (this.weapon === 'thunder') {
            ...
            if (this.thunderLoiMinhTimer > 0) {
                if (inp.s && now - this.lastShot > 300) {
                    this.fireThunderRaycast(now);
                    this.lastShot = now;
                }
                return;
            }
...
class BotAIController {
    ...
    renderBotWeaponModal() {
        ...
        const currentBotWeapon = p2 ? p2.weapon : 'basic';

        MODULES.BODY.forEach(mod => {
            const item = document.createElement('div');
            item.className = 'bot-weapon-item' + (mod.id === currentBotWeapon ? ' current' : '');
            ...
            `;
            item.onclick = () => this.changeBotWeapon(mod.id);
